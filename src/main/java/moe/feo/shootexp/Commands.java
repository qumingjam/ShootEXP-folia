...
public class Commands implements TabExecutor {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        if (args.length == 1) {
            ...
            if ("toggle".startsWith(main) && sender instanceof Player) {
                list.add("toggle");
            }
            if ("gui".startsWith(main) && sender instanceof Player) {
                list.add("gui");
            }
            if (list.size() > 0) {
                return list;
        ...
        if (args.length == 2) {
            ...
            if ("toggle".equals(main) && sender instanceof Player) {
                if ("messages".startsWith(sub1)) {
                    list.add("messages");
                }
                if ("attack".startsWith(sub1)) {
                    list.add("attack");
                }
            }
            if (list.size() > 0) {
        ...
        // toggle 命令只需要 2 个参数，不允许第 3 个参数
        if (args.length == 3 && "toggle".equals(args[0])) {
            return new ArrayList<>(); // 返回空列表，阻止进一步补全
        }
        return null;
    }
        ...
        switch (args[0].toLowerCase()) {
            case "help": {
                ...
                if (sender instanceof Player) {
                    sender.sendMessage(Language.COMMAND_HELP_TOGGLE.getString());
                    sender.sendMessage(Language.COMMAND_HELP_GUI.getString());
                }
                break;
            }
            case "status": {
            ...
            case "toggle": {
                ...
                switch (option) {
                    case "messages": {
                        boolean current = status.isReceivingMessages();
                        status.setReceiveMessages(!current);
                        // 保存玩家数据
                        DataManager.savePlayerData(uuid);
                        if (!current) {
                            sender.sendMessage(Language.COMMAND_TOGGLE_MESSAGES_ON.getString());
                        } else {
                            sender.sendMessage(Language.COMMAND_TOGGLE_MESSAGES_OFF.getString());
                        }
                        break;
                    }
                    case "attack": {
                        boolean current = status.canBeAttacked();
                        status.setCanBeAttacked(!current);
                        // 保存玩家数据
                        DataManager.savePlayerData(uuid);
                        if (!current) {
                            sender.sendMessage(Language.COMMAND_TOGGLE_ATTACK_ON.getString());
                        } else {
                            sender.sendMessage(Language.COMMAND_TOGGLE_ATTACK_OFF.getString());
                        }
                        break;
                    }
                    default: {
            ...
            case "gui": {
                ...
                SettingsGUI.openGUI((Player) sender);
                break;
            }
            default: {
                sender.sendMessage(Language.COMMAND_INVALID.getString());
